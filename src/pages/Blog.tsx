...
export const Blog: React.FC = () => {
  ...
  return (
    <>
      <title>Blog de Desarrollo Web | Joaquín</title>
      <meta name="description" content="Artículos y noticias sobre las últimas tendencias en desarrollo web, tecnología, SEO y diseño. Aprende y mantente al día." />

      <div className="bg-slate-50 dark:bg-slate-900 min-h-screen py-12 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-4">Blog & Noticias</h1>
            <p className="text-xl text-gray-600 dark:text-gray-300">
              Últimas tendencias en desarrollo web, tecnología y diseño.
            </p>
            {isAdmin && (
              <div className="mt-4 p-2 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-800 dark:text-indigo-200 inline-block rounded-lg text-sm font-medium">
          ...
          {loading ? (
            ...
            <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 px-4 py-3 rounded relative text-center">
              {error}
            </div>
          ) : posts.length === 0 ? (
            <div className="text-center text-gray-500 dark:text-gray-400 py-12">
              ...
              {isAdmin && (
                <Link to="/admin" className="text-indigo-600 dark:text-indigo-400 hover:underline mt-2 inline-block">Crear el primero</Link>
              )}
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {posts.map((post) => (
                <div key={post.id} className="relative group h-full">
                  ...
                  {isAdmin && (
                    <div className="absolute top-3 right-3 z-20 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 dark:bg-slate-800/90 p-1.5 rounded-lg shadow-sm backdrop-blur-sm">
                      <button
                  ...
                  <Link to={`/blog/${post.slug || post.id}`} className="block h-full">
                    <article className="bg-white dark:bg-slate-800 rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-all duration-300 flex flex-col h-full border border-gray-100 dark:border-slate-700">
                      {post.image_url && (
                      ...
                      <div className="p-6 flex-1 flex flex-col">
                        ...
                        <div className="text-indigo-600 dark:text-indigo-400 font-semibold flex items-center mt-auto">
